...
| lshw -C network<br>lshw -C storage/lshw -C disk<br>lshw -C processor<br>lshw -C memory | List hardware configuration information | **description**: mô tả ngắn gọn thiết bị là gì.<br>**product**: tên model chính xác của linh kiện.<br>**vendor**: nhà sản xuất (intel, amd, realtek...).<br>**logical name**: tên định danh trong hệ thống (ví dụ: ổ cứng là /dev/sda, card mạng là eth0 hoặc enp0s3).<br>**configuration**:  chứa tên **driver** và các thông số vận hành (speed, latency, width). |
