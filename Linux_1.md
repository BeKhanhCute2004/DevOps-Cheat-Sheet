...
| `fdisk -l` | Display disk partitions, sizes, and types | **Device**: tên phân vùng (vd: `/dev/sda1`).<br>**Boot**: dấu `*` nghĩa là phân vùng này dùng để boot.<br>**Start/End**: vị trí sector bắt đầu và kết thúc.<br>**Sectors / Size**: kích thước phân vùng.<br>**Type**: loại phân vùng (Linux, swap, EFI, NTFS...).<br>**Lưu ý**: cần `sudo` để chạy lệnh này. |
